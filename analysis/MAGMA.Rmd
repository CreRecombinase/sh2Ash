...
gene_file <- "~/Downloads/data/FUMA/genes.txt"
snp_file <- "~/Downloads/data/FUMA/snps.txt"
```
...
# magma_gene_df <- distinct(magma_df,gene)
# write_tsv(magma_gene_df,"../data/magma_genes.txt")
```
...
snp_cause_df <- mutate(gene_df,IndSigSNP=str_split(IndSigSNPs,pattern=";")) %>% 
  ...
  select(-IndSigSNP) %>% 
  inner_join(select(cause_df,-seb1,-seb2,-delta_elpd)) %>%  
  inner_join(magma_df) %>%
...
prob_shared <- distinct(magma_snp_df,rsID,.keep_all = T) %>% 
