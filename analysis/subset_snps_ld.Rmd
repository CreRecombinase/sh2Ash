...
snp_info <- data.frame("block" = rep(1:length(ld), lens))

pvals_b1_c <- with(draw_c$dat, pnorm(abs(beta_hat_1/seb1), lower.tail = F)*2)
top_c_ix <- top_snps_pval_ld(pvals=pvals_b1_c, snp_info=snp_info, ld=ld, ld_thresh=0.1, pval_thresh=Inf)

pvals_b1_s <- with(draw_s$dat, pnorm(abs(beta_hat_1/seb1), lower.tail = F)*2)
top_s_ix <- top_snps_pval_ld(pvals=pvals_b1_s, snp_info=snp_info, ld=ld, ld_thresh=0.1, pval_thresh=Inf)
```
...
grid.arrange(plts$b, plts$q, ncol=2)
```
These posteriors are more peaked than the oracle posterior and farther from the prior but less than the posteriors using all SNPs and no weights. 
...
+ Estimate posterior and calculate $z$-score using LD pruned subset of SNPs

I ran 20 simulations for each of seven values of $q$. Below, I show the number of simulations for which $z < \Phi(\alpha)$ with $\alpha$ equal to 0.9, 0.95 and 0.975. The method using LD pruning has the best type 1 error across values of $\alpha$ for small values of $q$ but the worst power when $q = 1$. For $\alpha = 0.95$, the method using all snps and no weights has the same power as the method using LD pruned SNPs. However, when $\alpha=0.975$, the method using LD pruned SNPs detects only 15 of the 20 true positives while the method using all SNPs and no weights detects 18. 

<img src="num_pos_2017-11-26.png" width="700">
