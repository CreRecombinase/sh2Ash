...
+ `corr-o`: "Oracle" correlation method in which $\beta_1$ are known. $g_{20}$ and $g_{21}$ are still estimated in the same was as for `corr-est`. 
...
plot(level, t1e_corr_1, ylim=range(c(t1e_corr_1, t1e_corr_2)), 
     ylab="Type 1 Error Rate", xlab="Nominal level", 
     main="Correlation approach - type 1 error", type="b", col="blue")
points(level, t1e_corr_2, type="b", col="blue", pch=4)
legend("topleft", legend=c("complete independence - corr.", "co-regulation - corr."), 
...
In the oracle correlation method, $\beta_1$ is known. Unfortunately, this also gives slightly too large type 1 error. 

```{r}
#CI
corr_res1_o <- data.frame("lambda"=res1$corr[1, , 1], "se"=res1$corr[1, ,2])
corr_res1_o$lower <- corr_res1_o$lambda - qnorm(0.975)*corr_res1_o$se
corr_res1_o$upper <- corr_res1_o$lambda + qnorm(0.975)*corr_res1_o$se
#Co-regulation
corr_res2_o <- data.frame("lambda"=res2$corr[1, , 1], "se"=res2$corr[1, ,2])
corr_res2_o$lower <- corr_res2_o$lambda - qnorm(0.975)*corr_res2_o$se
corr_res2_o$upper <- corr_res2_o$lambda + qnorm(0.975)*corr_res2_o$se

#Plot
ggplot(corr_res1_o) + geom_errorbar(aes(x=1:100, ymax = upper, ymin=lower))+
        geom_point(aes(x=1:100, y=lambda)) + geom_hline(yintercept=0, linetype=2) + 
        ggtitle("Null - Complete Independence\n Correlation method lambda estimates and 95% confidence intervals") + 
        xlab("Repetition") + ylab("Lambda") + 
        theme_bw() + theme(panel.grid=element_blank())
ggplot(corr_res2_o) + geom_errorbar(aes(x=1:100, ymax = upper, ymin=lower))+
        geom_point(aes(x=1:100, y=lambda)) + geom_hline(yintercept=0, linetype=2) + 
        ggtitle("Null - some correlation\n Correlation method lambda estimates and 95% confidence intervals") + 
        xlab("Repetition") + ylab("Lambda") + 
        theme_bw() + theme(panel.grid=element_blank())
...
```{r}
#Average lambda estimate
mean(corr_res1_o$lambda)
mean(corr_res2_o$lambda)
#Type 1 error rate
mean(corr_res1_o$upper < 0 | corr_res1_o$lower > 0)
mean(corr_res2_o$upper < 0 | corr_res2_o$lower > 0)
```
...
level <- seq(0.01, 0.1, length.out=10)
thresh_two_sided <- qnorm(1-(level/2))
t1e_corr_1 <- sapply(thresh_two_sided, FUN=function(s){mean(abs(corr_res1_o$lambda/corr_res1_o$se) > s)})
t1e_corr_2 <- sapply(thresh_two_sided, FUN=function(s){mean(abs(corr_res2_o$lambda/corr_res2_o$se) > s)})
plot(level, t1e_corr_1, ylim=range(c(t1e_corr_1, t1e_corr_2)), 
     ylab="Type 1 Error Rate", xlab="Nominal level", 
     main="Correlation Oracle approach - type 1 error", type="b", col="blue")
points(level, t1e_corr_2, type="b", col="blue", pch=4)
legend("topleft", legend=c("complete independence - corr.", "co-regulation - corr."), 
       col=rep(c("black", "blue"), each=2), pch=rep(c(1, 4), 2))
abline(0, 1, lty=2)
